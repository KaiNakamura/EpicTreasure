...
RUN apt-get update && \
    apt-get -y install python3-dev python-pip && \
...
RUN pip install git+https://github.com/Gallopsled/pwntools && \
    cd /root/tools && \
...
#     apt-get -m update && \
#     apt-get install -y libc6-arm64-cross libcc6-dev-i386 \
